...
export default function ClientTracking() {
  ...
  const getProgressSteps = () => {
    const allSteps = [
      { key: 'pending', label: lang === 'ar' ? 'تم الإرسال' : 'Demande soumise', icon: Clock },
      { key: 'validated', label: lang === 'ar' ? 'تمت الموافقة' : 'Validée', icon: CheckCircle },
      { key: 'preparing', label: lang === 'ar' ? 'قيد التحضير' : 'En préparation', icon: Package },
      { key: 'in_transit', label: lang === 'ar' ? 'في الطريق' : 'En transit', icon: Truck },
      { key: 'completed', label: lang === 'ar' ? 'تم التوصيل' : 'Livrée', icon: Check },
    ];
    ...
    switch (status) {
      case 'pending':
        return {
          ...
          icon: Clock,
          message: lang === 'ar' ? 'طلبك قيد المراجعة من قبل التاجر' : 'Votre demande est en cours d\'examen par le marchand',
          action: lang === 'ar' ? 'في انتظار الموافقة' : 'En attente de validation'
        };
      case 'validated':
        return {
          ...
          icon: CheckCircle,
          message: lang === 'ar' ? 'تمت الموافقة على التبديل' : 'Votre échange a été approuvé',
      ...
      case 'preparing':
        return {
          ...
          icon: Package,
          message: lang === 'ar' ? 'منتجك الجديد قيد التحضير' : 'Votre nouveau produit est en cours de préparation',
      ...
      case 'in_transit':
        return {
          ...
          icon: Truck,
          message: lang === 'ar' ? 'طردك في الطريق' : 'Votre colis est en route',
      ...
      case 'completed':
        return {
          ...
          icon: CheckCircle,
          message: lang === 'ar' ? 'تم التبديل بنجاح' : 'Votre échange est terminé',
      ...
      case 'rejected':
        return {
          ...
          icon: X,
          message: lang === 'ar' ? 'تم رفض طلبك' : 'Votre demande a été refusée',
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center" dir={dir}>
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">{t('exchangeNotFound')}</h2>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-600">
                  ...
                  <span>
                    ...
                    {new Date(exchange.created_at).toLocaleDateString(lang === 'ar' ? 'ar-TN' : 'fr-FR', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}
          ...
          {exchange.status !== 'rejected' && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <h2 className="text-lg font-semibold text-slate-900 mb-6">
                {lang === 'ar' ? 'تقدم التبديل' : 'Progression de votre échange'}
              </h2>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {depot && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="text-sm text-slate-500">{lang === 'ar' ? 'الاسم' : 'Nom'}</div>
              ...
              {transporter && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="text-sm text-slate-500">{lang === 'ar' ? 'الشركة' : 'Société'}</div>
                      <div className="font-medium text-slate-900">{transporter.name}</div>
                    </div>
                    <div>
                      <div className="text-sm text-slate-500">{lang === 'ar' ? 'التواصل' : 'Contact'}</div>
                  ...
                  return (
                    <div key={item.id} className="flex gap-4">
                      ...
                      <div className="flex-1 pb-2">
                        <p className="font-medium text-slate-900">{getStatusLabel(item.status)}</p>
                        <p className="text-sm text-slate-600">
                          {new Date(item.created_at).toLocaleDateString(lang === 'ar' ? 'ar-TN' : 'fr-FR', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: '2-digit',
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-slate-900 mb-2">
                  {lang === 'ar' ? 'تحتاج مساعدة؟' : 'Besoin d\'aide ?'}
                </h3>
                <p className="text-slate-700 mb-4">
                  {lang === 'ar'
                    ? 'تواصل مع التاجر عبر الرسائل لأي استفسار حول تبديلك.'
                    : 'Contactez le marchand via la messagerie pour toute question sur votre échange.'
                  }
                </p>
                <Link
                  to={`/client/chat/${exchange.id}`}
